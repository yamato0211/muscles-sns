...
from schemas.comments import Comment, CommentPayload
...
async def create_comment(post_id: str, payload: CommentPayload, db: Session = Depends(get_db), user_id: str = Depends(get_current_user)):
    if user_id is None:
        raise HTTPException(status_code=403, detail="jwt_token is invalid!")
    comment = c.create_comments_by_ids(db,post_id,user_id,payload.content)
    return comment
