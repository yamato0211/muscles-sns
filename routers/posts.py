...
from cruds.posts import create_post_by_user_id, get_timeline, get_posts_by_user_id, delete_post_by_post_id
...
async def delete_post(post_id: str, db: Session = Depends(get_db), user_id: str = Depends(get_current_user)):
    if user_id is None:
        raise HTTPException(status_code=403, detail="jwt_token si invarid!")
